...
do 
     ...
     #nzsql -h $1 -d $2 -u $3 -pw $4 -F',' -r -A -t -f $f -o $output - [EC] this command had an option to remove the header
     nzsql -h $1 -d $2 -u $3 -pw $4 -F',' -r -A -f $f -o $output
done;

# Remove header row 2 with dashes
# sed -i '/-[^-]*-/d' $Resultsfolder/*.csv - [EC] the dashed line is not being generated so replacing the dashes is not needed
cd $rootfolder

# Check for output contents on the last file generated
#if [ ! -d $output ] - [EC] this test checks if the file is a directory
if [ ! -s $output ]
then
