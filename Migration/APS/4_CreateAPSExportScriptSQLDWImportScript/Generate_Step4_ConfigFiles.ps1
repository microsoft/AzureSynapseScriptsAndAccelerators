...
ForEach ($csvItem in $configFileDriverFile ) 
{
	...
	elseif ($name -eq 'SqldwCopyScriptsFolder') { $SqldwCopyScriptsFolder = $value }
...
foreach ($dbName in $dbNames)
{
	...
	foreach ($key in $inFilePaths.Keys)
	{
		...
		foreach ($f in Get-ChildItem -path $inFileFolder  -Filter *dsql)
		{
			...
			$sqldwCopyScriptsFolderPerDb = -join($SqldwCopyScriptsFolder,$dbName,"\")
			...
			$row | Add-Member -MemberType NoteProperty -Name "CopyFilePath" -Value $sqldwCopyScriptsFolderPerDb -force
